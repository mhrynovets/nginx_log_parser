...
# KEY_PATH="/root/.ssh/id_container"
# OUTPUT_FILE="logs.csv"
...
# # 2. SSH Setup - copy keys, ensure correct access, extract service domain and add to known hosts
# if [ -f "/tmp/keys/$SSH_KEY_NAME" ]; then
#     mkdir -p /root/.ssh && chmod 700 /root/.ssh
#     cp "/tmp/keys/$SSH_KEY_NAME" "$KEY_PATH"
#     chmod 600 "$KEY_PATH"
#     DOMAIN=$(echo "$GIT_REMOTE_URL" | sed -e 's/.*@//' -e 's/[:/].*//')
#     ssh-keyscan -t rsa,ed25519 -H "$DOMAIN" >> /root/.ssh/known_hosts 2>/dev/null
#     export GIT_SSH_COMMAND="ssh -i $KEY_PATH -o BatchMode=yes"
# else
#     echo "CRITICAL: Key $SSH_KEY_NAME not found."
#     exit $ERR_SSH_KEY_MISSING
# fi
...
# echo "--- Pushing results ---"
# git add "$OUTPUT_FILE"
# git commit -m "Auto-update ($TARGET_BRANCH): $(date +%T)"
# git push -u origin "$TARGET_BRANCH" || exit $ERR_GIT_OPS
